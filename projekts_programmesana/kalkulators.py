...
window = Tk()
window.title('Fizikas apgaismojuma formulu kalkulators') # To give the title of the window.
window.geometry("400x470") # To initialize the size of the window. 
...
mainTitle.place(x=10, y=10) # Use the place layout to place further labels
...
def pirma_formula():
    ...
    VALUES(?, ?, ?, ?, ?) '''
    c.execute(sql, (ieraksta_id, E, F, S, REZ))
    conn.commit()
    resultEntry.delete(0, END)
    resultEntry.insert(0, str(rezultats))
...
def vestures_tabula():
    ...
    res = c.execute("SELECT * FROM pirma_formula")
...
resultEntry = Entry(window, bg = "green")
resultEntry.place(x=200, y=110, height=20, width=143)

resultEntry1 = Entry(window, bg = "green")
